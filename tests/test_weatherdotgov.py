...
from volttron.utils import format_timestamp, parse_timestamp_string, process_timestamp
    ...
    for x in range(0, len(query_data)):
        ...
        if results:
            ...
            for record in results:
                forecast_time = parse_timestamp_string(record[0])
    ...
    for x in range(0, len(cache_data)):
        ...
        if cache_location_data.get("weather_results"):
            ...
            for y in range(0, len(query_weather_results)):
                ...
                query_time, oldtz = process_timestamp(result[0])
                query_time = format_timestamp(query_time)
