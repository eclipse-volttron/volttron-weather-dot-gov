...
import grequests  # Monkey patches ssl before import of requests.
import logging
import re
import requests  # Must be imported after grequests due to ssl monkey patch.
import sys
...
from volttron.utils import jsonapi, load_config, update_kwargs_with_config, vip_main

from volttron.weather.base import BaseWeatherAgent, doc_inherit
...
def weather_agent(config_path, **kwargs):
    ...
    update_kwargs_with_config(kwargs, config_dict)
...
class WeatherDotGovAgent(BaseWeatherAgent):
    ...
    def get_point_name_defs_file(self):
        ...
        # text mode.
        return str(get_resource_files("weather_dot_gov").joinpath("data/name_mapping.csv"))
    ...
    def validate_location_formats(self, accepted_formats, location):
        """
        Regular expression comparison to validate the various location
        dictionary formats
    ...
    def query_current_weather(self, location):
        ...
        g_response = self.make_web_request(url)
        try:
            response = jsonapi.loads(g_response.content)
            properties = response["properties"]
            observation_time = properties["timestamp"]
            return observation_time, properties
        except ValueError:
            self.generate_response_error(url, g_response.status_code)

    @doc_inherit
    def query_forecast_service(self, service, location, quantity, forecast_start):
        """
        Returns forecast weather from Weather.gov.
    ...
    def query_hourly_forecast(self, location):
        ...
        if location.get('station'):
            ...
            g_response = self.make_web_request(url)
            try:
                _log.info(f"{g_response.content}")
                response = jsonapi.loads(g_response.content)
        ...
        g_response = self.make_web_request(url)
        return self.extract_forecast_data(url, g_response)
    ...
    def get_forecast_url(self, location):
        ...
        g_response = self.make_web_request(url)
        try:
            _log.info(f" after get forecast url for lat lon: response is{g_response.content}")
            response = jsonapi.loads(g_response.content)
    ...
    def extract_forecast_data(self, url, g_response):
        try:
            response = jsonapi.loads(g_response.content)
            data = []
            properties = response["properties"]
...
def main():
    ...
    try:
        vip_main(weather_agent, version=__version__)
